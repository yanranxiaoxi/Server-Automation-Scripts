...
# 创建系统定时任务
echo "${timerTime} root curl -fsSL https://sh.soraharu.com/ServerMaintenance/Backup/backupPostgreSQL14DatabaseToS3.sh | bash -s -- \"${serverName}\" \"${containerName}\" \"${databaseUser}\" \"${databasePassword}\" \"${s3BucketName}\" \"${s3StorageClass}\" \"${timerTime}\"" >/etc/cron.d/backupPostgreSQL14DatabaseToS3."${containerName}".cron
