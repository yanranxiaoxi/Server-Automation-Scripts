...
# Backup - backupPostgreSQL14DatabaseToS3
#
# 将 PostgreSQL 14 数据库备份到 S3 存储桶
#
...
# 创建系统定时任务
echo "${timerTime} root wget -O ~/backupPostgreSQL14DatabaseToS3.sh https://sh.soraharu.com/ServerMaintenance/Backup/backupPostgreSQL14DatabaseToS3.sh && sh ~/backupPostgreSQL14DatabaseToS3.sh \"${serverName}\" \"${containerName}\" \"${databaseUser}\" \"${databasePassword}\" \"${s3BucketName}\" \"${s3StorageClass}\" \"${timerTime}\" && rm -f ~/backupPostgreSQL14DatabaseToS3.sh" >/etc/cron.d/backupPostgreSQL14DatabaseToS3."${containerName}".cron
